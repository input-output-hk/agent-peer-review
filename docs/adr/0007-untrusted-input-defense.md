...
The read-only default was originally written as conditional, gated by a `runChecks` config switch. No code path ever read that switch, so it was removed from the schema rather than wired up (see issue #55), and the skills now instruct an unconditional read-only review. The decision is unchanged and slightly stronger: there is no supported way to opt into executing the code under review, which is the safer reading given the diff is untrusted input.
