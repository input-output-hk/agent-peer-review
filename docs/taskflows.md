...
Your own open pull requests. For each one, in order: `pr_stabilize` syncs the branch; the implementing agent reviews the exact clean head and calls `pr_self_review` only after fixing everything its pass found; `pr_expedite` asks the gate for a verdict; and a refusal reaches `pr_request_review` only after that successful self-review. The author-owned request operation returns `self-review-required` without consuming a peer's queue if the current head has no authenticated pass.

If prior review work has become disproportionate while the current implementation is safe, the implementer may call `pr_create_followup`. It creates or returns the only review follow-up issue allowed for that PR. The issue must own stable finding IDs, explain the concrete problem and why deferral is proportionate, and carry bounded acceptance criteria. The self-review asks the peer to approve with that issue taken into account. A follow-up cannot hide a current blocker, and a second follow-up issue is refused.

The reviewer step still keys on the gate's refusal itself, not which rail refused. The durable state is a self-review marker keyed to the head, an optional single follow-up link, a proposal keyed to the head, and the native review request. Conflicts escalate; closed/merged PRs and drafts stop; `blocked` continues because a protected PR awaiting review commonly reports that state.
...
**pi-taskflow has no scheduler.** Nothing in this package or in pi-taskflow makes a flow recur; a run happens because something asked for one. To sweep on a cadence, drive it from outside. Each flow is safe to re-run: `pr_stabilize` is a no-op on an up-to-date branch, `pr_self_review` and proposals are idempotent at the current head, `pr_create_followup` returns the existing issue, and `pr_request_review` will not ask a second time for the same round.
...
| `no peer was asked because the current head has no successful Self-review` | The author has not recorded a passing exact-head self-review. | Fix any issue found, rerun the pass, and call `pr_self_review` before requesting the peer. |
