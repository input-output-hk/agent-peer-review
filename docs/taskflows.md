...
The refusal is limited to the same bot allowlist the steward accepts, and that limit matters in both directions. A bot outside it, a codegen or release bot say, opens pull requests carrying real source changes that no automated path here may merge, so those are requested for peer review exactly like a person's would be. And the flip side: keep a flow's `botAuthors` list in step with that allowlist, or a pull request the requester declines to hand over can be one the steward never discovers.
