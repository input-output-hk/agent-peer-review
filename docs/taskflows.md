...
One pull request this flow never hands to a peer is a **bot-authored** one. `pr_request_review` returns `bot-authored` and writes nothing at all: no label, no request. GitHub only forbids approving your *own* pull request, so your agent may review and approve a bot's itself, and that is `pr-steward`'s job. Asking another engineer's agent to look at a machine-checkable dependency bump adds a round trip and a person's queue for no gain.
...
- **Its own approval counts toward branch protection.** On a repository that requires an approving review, the required approval cannot be present before the operation whose whole job is to add it has run, so rail 5 counts the approval this call is about to submit. It adds exactly one: a repository requiring two approvals with none present still holds the change back, correctly. See [The safety model](#the-safety-model).
- **The merge is re-checked after approving.** The tool approves, re-reads protection, mergeability, and the checks without that pending-approval allowance, and merges only if the state that now really exists says it may. When it does not, the result is `approved`: the approval landed and the merge did not. That is a real outcome rather than a failure, because an approval is durable and unblocks the pull request for whoever merges it next, and the next run tries the merge again.
- **A dependency-specific size policy.** 10 files and 4000 changed lines, instead of the general 200-line cap, because a lockfile's line count is mechanical churn rather than reviewable surface. The content check is what bounds the risk here: every changed manifest line must be a paired dependency version edit, and every other changed file must be a lockfile.
...
**The ten-rail gate decides, not the model.** `pr_expedite` and `pr_approve_dep_upgrade` hand every input to one central gate, which merges only when all ten rails agree: the change classifies into the docs, lint, CI, or dependency allowlist and touches no source or test path; it fits the size caps (10 files and 200 lines by default, 10 files and 4000 lines for a dependency upgrade); required checks are green; GitHub's own mergeable state is clean; branch protection is satisfied; the security-alert rail is satisfied; no human review is in flight; the autonomy for this call is `auto`; the head SHA has not moved since the evaluation; and, when the decision is an approval (`pr_approve_dep_upgrade`), the acting login is not the pull request's own author. Any single "no" turns the decision into a proposal, with the reason quoted.
...
**Rail 5 counts the approval the approver is about to add.** Branch protection is read before anything is written, so on a repository that requires an approving review the required approval is by definition absent at that moment, and demanding it there would make the operation that supplies the approval unable to satisfy the requirement its own approval exists to satisfy. So when the decision **is** an approval, rail 5 counts that one pending approval, and only then. Everything about the rail that could hide a real problem is unchanged: it adds exactly one, so two required approvals with none present still fails; protection that cannot be read at all still fails closed; required conversation resolution still fails closed; required checks must still be green; a nonsensical approval count is still rejected before the increment is applied; and the allowance is withheld entirely when the acting login is the author (whose approval GitHub would refuse) or already holds a standing approval (which is counted once, not twice). `pr_expedite` merges rather than approves and passes no such claim, so its rail 5 is exactly what it always was.

Because that allowance describes a state which does not yet exist, it authorizes the approval and nothing more. `pr_approve_dep_upgrade` approves first, then re-reads protection, mergeability, and the checks **without** the allowance, and merges only if the real post-approval state permits it. When it does not, the result is `approved` rather than `approved-and-merged`, naming what still blocks.
...
**Autonomy is gated on more than the flag.** The open questions that have to be answered before any repository should run in auto are tracked in [issue #39](https://github.com/input-output-hk/agent-peer-review/issues/39). Two of them are now answered: how a required approving review is satisfied by the operation that supplies the approval (rail 5 above, with the post-approval re-check), and the dependency-specific size policy. The stale-approval policy, ruleset-protected branches, and the rest are still open. Until those are closed, treat `autonomy=auto` as an experiment you are supervising, not a mode you leave on.

:::note
`pr-steward` used to be held back twice over on exactly the repositories it is for: a required approving review made rail 5 unsatisfiable for the one operation whose approval would satisfy it, and a realistic lockfile diff of thousands of lines failed the shared 200-line size rail. Both are addressed above, and neither loosening is a bypass: the approval is a real action GitHub counts, the merge is re-checked afterwards, and the dependency size policy leaves the file cap and every other rail alone. Propose mode is unaffected either way, since it posts the same verdict.
:::
