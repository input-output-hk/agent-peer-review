...
function main() {
  ...
  if (items.length === 0) {
    process.stdout.write("pr-reviewer: no review requests and nothing to follow up on.\n");
    return;
  }
  ...
  for (const item of items) {
    ...
    else if (action === "wait") counts.waiting += 1;
    else if (action === "hold-for-human") counts["held-for-human"] += 1;
    else if (action === "abandoned") counts.abandoned += 1;
  ...
  process.stdout.write(`pr-reviewer: ${items.length} pull request(s). ${line}\n`);
