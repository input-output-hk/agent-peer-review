...
2. Read the `action` it returns and report it verbatim:
   ...
   - `approved`: the approval was posted and stands on the pull request, but the merge did not happen. This is its own outcome, not a softer `approved-and-merged` and not a `proposed`: an approval is durable and useful on its own, because it unblocks the pull request for whoever merges it next. `reasons` says what still blocks the merge, for example a second required approval, a required check that is not green yet, a mergeable state that is not clean, or a head commit that moved. Report it as `approved` and let the next run try the merge again.
